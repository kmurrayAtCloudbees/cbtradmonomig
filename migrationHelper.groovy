pipeline {
    ...
    stages {
        ...
        stage('Plan Job Grouping (PLAN_DRY_RUN)') {
            ...
            steps {
                echo "[PLAN MODE] Generating job status groupings (active.txt, inactive.txt, combined.txt)..."
                echo "[PLAN MODE] Would analyze current jobs on source controller"
                echo "[PLAN MODE] Would derive assignments.csv for target routing"
                echo "[PLAN MODE] Would commit results to Git repo or output to console"
            }
        }
